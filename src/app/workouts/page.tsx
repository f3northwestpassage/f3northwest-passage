...
export async function generateMetadata(): Promise<Metadata> {
  ...
  try {
    ...
    console.error(
      'Error fetching locale data for workouts page metadata:',
      error
    );
  }
  ...
  return {
    ...
    keywords: [
      ...
      'bootcamp workouts Jersey Village',
      'bootcamp workouts Bridgeland',
      'bootcamp workouts Hockley',
      'outdoor training Houston',
      'outdoor training Cypress',
      'outdoor training Jersey Village',
      'outdoor training Bridgeland',
      'outdoor training Hockley',
      'free fitness groups',
      'Cypress exercise groups',
      'Houston exercise groups',
      'Jersey Village exercise groups',
      'Bridgeland exercise groups',
      'Hockley exercise groups',
      ...
      'early morning workouts Jersey Village',
      'early morning workouts Bridgeland',
      'early morning workouts Hockley',
    ],
    openGraph: {
...
export default async function Page() {
  let rawWorkouts: WorkoutClean[] = [];
  let rawLocations: LocationClean[] = [];

  try {
    [rawWorkouts, rawLocations] = await Promise.all([
      fetchWorkoutsData(),
      fetchLocationsData(),
    ]);
  } catch (error) {
    console.error('Error fetching workouts/locations data:', error);
  }
  ...
  return (
    <>
      ...
      <main className="bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100">
        ...
        <section className="bg-gray-800 dark:bg-gray-950 py-12 px-4 text-white text-center">
          <h2 className="text-4xl font-extrabold mb-4">
            FREE OUTDOOR WORKOUT LOCATIONS IN{' '}
            {locales?.region_city?.toUpperCase() || 'CYPRESS & HOUSTON'}
          </h2>
          <p className="text-lg opacity-90 mb-3 max-w-2xl mx-auto">
            Free outdoor workouts held rain or shine, no membership fees
            required. Open to men of all ages and fitness levels.
          </p>
            ...
            <div className="space-y-6">
              {sortedLocations.length === 0 && rawWorkouts.length === 0 ? (
                ...
                sortedLocations.map((location) => {
                  const workoutsAtThisLocation: WorkoutClean[] =
                    groupedWorkouts[location._id] || [];

                  return (
                    <Link
                      href={`/workouts/${encodeURIComponent(location.name)}`}
                      key={location._id}
                      passHref
                    >
                      ...
                      >
                        ...
                        <div className="mb-4 pb-3 border-b border-gray-200 dark:border-gray-700">
                          ...
                          {(location.embedMapLink || location.mapLink) && (
                            <div className="flex justify-center">
                              {' '}
                              {/* Use flex justify-center on the div holding the button(s) */}
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-300 dark:border-gray-700">
                          <h4 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4 text-center">
                            Workout Schedule:
                          </h4>
                          <div
                            className={
      ...
      <Footer
        regionName={locales.region_name ?? ''}
        regionFacebook={locales.region_facebook ?? ''}
        regionInstagram={locales.region_instagram ?? ''}
        regionLinkedin={locales.region_linkedin ?? ''}
        regionXTwitter={locales.region_x_twitter ?? ''}
      />
    </>
  );
}
