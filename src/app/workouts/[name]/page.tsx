...
import type {
  ...
  LocationClean,
  LocaleData,
} from '../../../../types/workout'; // Import WorkoutClean type from where it's defined
...
export async function generateStaticParams() {
  ...
  try {
    ...
    console.error(
      'Error fetching locations for generateStaticParams in workouts/[name]/page.tsx:',
      error
    );
    return [];
  }
...
export async function generateMetadata({
  ...
  try {
    ...
    console.error(
      `Error fetching location for metadata of ${decodedName}:`,
      error
    );
    return {
  ...
  return {
    ...
    description:
      ...
      `Workout details for F3 ${location.name}. Join us at ${location.address}.`,
  };
}
...
export default async function LocationPage({
  ...
  const location: LocationClean | undefined = allLocations.find(
    (loc) => loc.name === decodedName
  );
  if (!location) {
    notFound();
  }
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl p-6 text-gray-900 dark:text-gray-100 mb-8">
          ...
          <div className="mb-4 text-center">
            ...
            {location.description && (
              <p className="text-gray-700 dark:text-gray-300 mb-4 text-md leading-relaxed">
                {location.description}
              </p>
            )}
              ...
              >
                {workoutsAtThisLocation.map((workout) => (
                  <WorkoutCard
                    key={
                      workout._id ||
                      `${location._id}-${workout.startTime}-${workout.days?.[0]}`
                    }
                    _id={workout._id || ''}
