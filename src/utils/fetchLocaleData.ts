...
export async function fetchLocaleData(): Promise<LocaleData> {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `[fetchLocaleData] API fetch error: Status ${response.status} - ${response.statusText} for ${apiUrl}`
      );
    ...
    return {
      ...
      region_map_lat:
        (typeof data.region_map_lat === 'string'
          ? parseFloat(data.region_map_lat)
          : data.region_map_lat) ?? mockRegion.region_map_lat,
      region_map_lon:
        (typeof data.region_map_lon === 'string'
          ? parseFloat(data.region_map_lon)
          : data.region_map_lon) ?? mockRegion.region_map_lon,
      region_map_zoom: data.region_map_zoom ?? mockRegion.region_map_zoom,
      region_map_embed_link:
        data.region_map_embed_link ?? mockRegion.region_map_embed_link,
      region_logo_url: data.region_logo_url ?? mockRegion.region_logo_url,
      region_hero_img_url:
        data.region_hero_img_url ?? mockRegion.region_hero_img_url,
      region_google_form_url:
        data.region_google_form_url ?? mockRegion.region_google_form_url,
      region_fng_form_url:
        data.region_fng_form_url ?? mockRegion.region_fng_form_url,
    };
  } catch (error) {
    console.error(
      `[fetchLocaleData] FETCH_LOCALE_DATA_ERROR for ${apiUrl} (returning mock):`,
      error
    );
